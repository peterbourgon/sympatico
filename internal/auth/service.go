...
func NewService(repo Repository, eventsTotal *prometheus.CounterVec) *Service {
	return &Service{
		repo:        repo,
...
func (s *Service) Signup(ctx context.Context, user, pass string) (err error) {
	defer func() {
		ctxlog.From(ctx).Log("auth_method", "Signup", "signup_user", user, "signup_err", err)
		s.eventsTotal.WithLabelValues("signup", fmt.Sprint(err == nil)).Add(1)
	}()
...
func (s *Service) Login(ctx context.Context, user, pass string) (token string, err error) {
	defer func() {
		ctxlog.From(ctx).Log("auth_method", "Login", "login_user", user, "login_err", err)
		s.eventsTotal.WithLabelValues("login", fmt.Sprint(err == nil)).Add(1)
	}()
...
func (s *Service) Logout(ctx context.Context, user, token string) (err error) {
	defer func() {
		ctxlog.From(ctx).Log("auth_method", "Logout", "logout_user", user, "logout_err", err)
		s.eventsTotal.WithLabelValues("logout", fmt.Sprint(err == nil)).Add(1)
	}()
...
func (s *Service) Validate(ctx context.Context, user, token string) (err error) {
	defer func() {
		ctxlog.From(ctx).Log("auth_method", "Validate", "validate_user", user, "validate_err", err)
		s.eventsTotal.WithLabelValues("validate", fmt.Sprint(err == nil)).Add(1)
	}()
